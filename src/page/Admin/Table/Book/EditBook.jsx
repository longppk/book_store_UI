...
function EditBook() {
    ...
    const [listOldImages, setListOldImages] = useState([]);
    ...
    useEffect(() => {
        const fetchBookInfo = async () => {
            const res = await bookServices.getBookInfo(bookId);
            const listGenres = await bookServices.getListGenresByBookId(bookId);
            const listAuthors = await bookServices.getListAuthorsByBookId(bookId);
            const _listOldImages = await bookServices.getAllOldImages(bookId);
            setListOldImages(_listOldImages);
    ...
    const handleSubmit = (e) => {
        ...
        if (!formValid) {
            ...
            const submitAddBookForm = async (formData) => {
                const res = await bookServices.editBook(bookId, formData);
                    ...
                    <form className="mx-auto">
                        ...
                        <div className="mb-5">
                            ...
                            <div className="my-5 flex gap-x-5">
                                ...
                                {listOldImages?.length !== 0 &&
                                    listOldImages.map(({ id, url }) => (
                                        <img
                                            key={id}
                                            className="h-96 w-60 object-contain rounded-xl border-2 drop-shadow-md"
                                            src={url}
