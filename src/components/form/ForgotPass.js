...
function ForgotPass() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res2 = await axios.get(`http://localhost:8080/mail/valid/${values.username}/${values.validCode}`);
            if (res2.data === 200) {
    ...
    const handleSendMail = async (e) => {
        ...
        try {
            const res = await axios.post(`http://localhost:8080/mail/send/${values.username}`);
                    ...
                    {show ? (
                        show && (
                            <>
                                ...
                                <button onClick={handleSendMail} className="btn-submit" disabled={!isClickable}>
